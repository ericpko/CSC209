...
/* Return a dynamically allocated string of each TA and which student
 * they are serving.
 */
char *print_currently_serving(Ta *ta_list) {
    ...
    // count the size of the dynamic string needed
    int size = 1;           // for '\0'
    Ta *cur = ta_list;
    while (cur != NULL) {
        if (cur->current_student != NULL) {
            size += 19 + strlen(cur->name) + strlen(cur->current_student->name);
        } else {
            size += 21 + strlen(cur->name);
        }
        cur = cur->next;
    }

    // allocate the string and start appending
    char *curr_serving;
    cur = ta_list;
    if (cur == NULL) {
        size = strlen("No TAs are currently working.\r\n") + 1;
        if ((curr_serving = malloc(size * sizeof(char))) == NULL) {
            perror("malloc");
            exit(EXIT_FAILURE);
        }
        strncpy(curr_serving, "No TAs are currently working.\r\n",
                                                    size * sizeof(char) - 1);
        return curr_serving;
    }
    ...
    char *temp;
    memset(curr_serving, '\0', size * sizeof(char));
    while (cur != NULL) {
        if (cur->current_student != NULL) {
            asprintf(&temp, "TA: %s is serving %s.\r\n",
                                        cur->name, cur->current_student->name);
        } else {
            asprintf(&temp, "TA: %s has no student\r\n", cur->name);
        }
        strncat(curr_serving, temp, size * sizeof(char) -
                                                    strlen(curr_serving) - 1);
        free(temp);
        cur = cur->next;
    }
    curr_serving[size - 1] = '\0';
    return curr_serving;
}

/* Returns a dynamically allocated string list of all the students in the
 * queue. This does not list students currently being served.
 */
char *print_full_queue(Student *stu_list) {
    ...
    int size = strlen("Full Queue\r\n") + 1;  // extra 1 for '\0'
    for (Student *cur = stu_list; cur != NULL; cur = cur->next_overall) {
        size += (11 + strlen(cur->name) + strlen(cur->course->code));
    }

    char *stu_queue = malloc(size * sizeof(char));
    if (stu_queue == NULL) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
    strncpy(stu_queue, "Full Queue\r\n", size * sizeof(char) - 1);
    for (Student *cur = stu_list; cur != NULL; cur = cur->next_overall) {
        strncat(stu_queue, "Student ",
                                    sizeof(stu_queue) - strlen(stu_queue) - 1);
        strncat(stu_queue, cur->name,
                                    sizeof(stu_queue) - strlen(stu_queue) - 1);
        strncat(stu_queue, ":", sizeof(stu_queue) - strlen(stu_queue) - 1);
        strncat(stu_queue, cur->course->code,
                                    sizeof(stu_queue) - strlen(stu_queue) - 1);
        strncat(stu_queue, "\r\n", sizeof(stu_queue) - strlen(stu_queue) - 1);
    }
    stu_queue[size - 1] = '\0';
