...
void write_random_pieces(int client_soc, const char *message, int times);
...
int main(int argc, char **argv) {
    ...
    int client_soc = connect_to_server(PORT, argv[1]);

    write_random_pieces(client_soc, MESSAGE, TIMES);
...
void write_random_pieces(int client_soc, const char *message, int times) {
    ...
    while (current_byte < total_bytes) {
        ...
        write(client_soc, piece, piece_size);
